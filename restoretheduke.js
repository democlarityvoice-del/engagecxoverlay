// ===== EngageCX minimal debug wrapper (non-destructive) =====
;(function initEngageCXLoader() {
  try {
    ...
    if (!jq || !jq.fn || !jq.fn.jquery) {
      return void setTimeout(initEngageCXLoader, 500); // retry until jQuery is ready
    }

    // Prevent double init across SPA navs
    ...
    // Basic debug context
    window.__engagecx_debug__ = {
      jquery: jq.fn.jquery,
      hasNav: !!jq('#nav-buttons').length,
      hasContent: !!jq('#content').length
    };
    console.info('[EngageCX] init with jQuery', jq.fn.jquery, window.__engagecx_debug__);
    ...
    setTimeout(initEngageCXLoader, 1000); // retry on error
  }
})();
