// ===== EngageCX: safe bootstrap (waits for jQuery + nav; no behavior changes) =====
;(function initEngageCXLoader() {
  try {
    console.info('[EngageCX] loader tick', Date.now());

    var $ = window.jQuery || window.$;
    if (!$ || !$.fn || !$.fn.jquery) {
      // jQuery not ready yet — try again shortly
      return void setTimeout(initEngageCXLoader, 300);
    }

    if (window.__engagecx_init_done__) {
      console.info('[EngageCX] already initialized, skipping');
      return;
    }

    // Wait helper: runs fn once pred() becomes true. No arbitrary cutoff.
    function when(pred, fn) {
      ...
      // gentle interval backstop (stops once pred() passes)
      let tries = 0;
      const iv = setInterval(function () {
        if (pred() || ++tries > 600) clearInterval(iv); // ~3 minutes safety
        if (pred()) { clearInterval(iv); fn(); }
      }, 300);
    }

    // We’ll start only once we can actually see the nav container and at least one li (or #nav-music)
    when(function () {
      return $('#nav-buttons').length &&
             ( $('#nav-music').length || $('#nav-buttons').children('li').length );
    }, function startEngageCX() {
      window.__engagecx_init_done__ = true;
      console.info('[EngageCX] init with jQuery', $.fn.jquery, {
        hasNav: !!$('#nav-buttons').length,
        hasMusic: !!$('#nav-music').length,
        navItems: $('#nav-buttons').children('li').length
      });

      // === BEGIN: your original "worked until an hour ago" code (unchanged) ===
    ...
    console.error('[EngageCX] init error', err);
    setTimeout(initEngageCXLoader, 1000);
  }
})();
